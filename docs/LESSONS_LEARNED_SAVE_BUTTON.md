...
### 9. **Versioning & Self-Verification Blind Spot**

**Problem:**
- Behavior and UI changed, but the **extension version in Chrome never changed**.
- Root `package.json` stayed at `0.1.13`, so `chrome-extension/manifest.js` kept writing `"version": "0.1.13"` into `dist/manifest.json`.
- Turbo logs (`@extension/sidepanel@0.1.13`, `chrome-extension@0.1.13`) looked “normal”, making it easy to miss that we were still shipping the old version number.
- Chrome cached the loaded extension, so without a manual **Reload** in `chrome://extensions`, it looked like “nothing changed” even when code had changed.

**Why It Failed:**
- I changed behavior in `SidePanel.tsx`, `executor.ts`, and settings, but **never bumped the root version**.
- The build system correctly used `package.json.version` as the single source of truth, so every build kept the same version.
- There was **no visible version label in the side-panel UI**, so copying UI text or screenshots gave no clue which build was running.

**Fix:**
- Treat `package.json.version` as a required step for any behavior change that must be user-visible:
  - Bump version (e.g. `0.1.13` → `0.1.14`) before/with behavior changes.
  - Rebuild and then **Reload** the extension from `dist/` in `chrome://extensions`.
- Confirm that `dist/manifest.json` shows the new version and that Chrome’s extensions page matches it.
- Add a visible version indicator in the side-panel header (wired to manifest version) so the running build ID is always visible in the UI.

**Lesson:** When debugging “nothing changed” complaints, always verify:
- The version in `package.json`.
- The version in `dist/manifest.json`.
- The version shown in `chrome://extensions` **after** hitting Reload.
- Prefer a visible version string in the UI so screenshots and copy/paste include the actual running build ID.
