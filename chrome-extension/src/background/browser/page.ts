...
export default class Page {
  ...
  async getElementScrollInfo(elementNode: DOMElementNode): Promise<[number, number, number]> {
    ...
    if (!element) {
      throw new Error(`Element: ${elementNode.getDescription()} not found`);
    }
  ...
  async inputTextElementNode(useVision: boolean, elementNode: DOMElementNode, text: string): Promise<void> {
    ...
    try {
      ...
      // Wrap in try-catch to handle detached elements
      let tagName: string;
      let isContentEditable: boolean;
      let isReadOnly: boolean;
      let isDisabled: boolean;

      try {
        tagName = await element.evaluate(el => el.tagName.toLowerCase());
        isContentEditable = await element.evaluate(el => {
          if (el instanceof HTMLElement) {
            return el.isContentEditable;
          }
          return false;
        });
        isReadOnly = await element.evaluate(el => {
          if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) {
            return el.readOnly;
          }
          return false;
        });
        isDisabled = await element.evaluate(el => {
          if (el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement) {
            return el.disabled;
          }
          return false;
      ...
      const elementDesc = elementNode.getDescription();
      const errorMsg = `Failed to input text into element: ${elementDesc}. Error: ${error instanceof Error ? error.message : String(error)}`;
