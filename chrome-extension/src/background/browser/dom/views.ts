...
export class DOMElementNode extends DOMBaseNode {
  ...
  /**
   * Get a useful string representation of this element for error messages
   */
  getDescription(): string {
    const parts: string[] = [];
    if (this.highlightIndex !== null) {
      parts.push(`index=${this.highlightIndex}`);
    }
    if (this.tagName) {
      parts.push(`tag=${this.tagName}`);
    }
    ...
    const text = this.getAllTextTillNextClickableElement(2).trim();
    if (text) {
